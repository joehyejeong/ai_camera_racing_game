...
bkcore.hexgl.Gameplay = function (opts) {
	...
	this.timeLimit = opts.timeLimit == undefined ? null : opts.timeLimit;
	this.timeLimitMessageShown = false;
	...
	this.modes.timeattack = function () {
		...
		// Check if reached finish line (from start to last checkpoint)
		if (cp == self.track.checkpoints.last && self.previousCheckPoint == self.track.checkpoints.start) {
			// Finish! Record the time and end the race
		...
		else if (cp == self.track.checkpoints.start && self.previousCheckPoint == self.track.checkpoints.last) {
			// Completed a full lap (last -> start)
			self.previousCheckPoint = cp;
			var t = self.timer.time.elapsed;
			self.lapTimes.push(t - self.lapTimeElapsed);
			self.lapTimeElapsed = t;
...
bkcore.hexgl.Gameplay.prototype.update = function () {
	...
	if (this.step == 0 && this.timer.time.elapsed >= this.countDownDelay + this.startDelay) {
		...
		if (this.timeLimit != null && this.timer.time.elapsed >= this.timeLimit) {
			if (this.hud != null && !this.timeLimitMessageShown) {
				this.hud.display("TIME OVER", 1);
				this.timeLimitMessageShown = true;
			}
			this.shipControls.destroy();
